...
class BMI270
{
  ...
  private:
    ...
    int8_t extractFIFOData(BMI270_SensorData *data, bmi2_fifo_frame *fifoData, uint16_t *numFrames,
                           uint8_t sensorSelect);

    // Read/write helper functions
    static BMI2_INTF_RETURN_TYPE readRegisters(uint8_t regAddress, uint8_t *dataBuffer, uint32_t numBytes,
                                               void *interfacePtr);
    static BMI2_INTF_RETURN_TYPE readRegistersI2C(uint8_t regAddress, uint8_t *dataBuffer, uint32_t numBytes,
                                                  BMI270_InterfaceData *interfaceData);
    static BMI2_INTF_RETURN_TYPE readRegistersSPI(uint8_t regAddress, uint8_t *dataBuffer, uint32_t numBytes,
                                                  BMI270_InterfaceData *interfaceData);
    static BMI2_INTF_RETURN_TYPE writeRegisters(uint8_t regAddress, const uint8_t *dataBuffer, uint32_t numBytes,
                                                void *interfacePtr);
    static BMI2_INTF_RETURN_TYPE writeRegistersI2C(uint8_t regAddress, const uint8_t *dataBuffer, uint32_t numBytes,
                                                   BMI270_InterfaceData *interfaceData);
    static BMI2_INTF_RETURN_TYPE writeRegistersSPI(uint8_t regAddress, const uint8_t *dataBuffer, uint32_t numBytes,
                                                   BMI270_InterfaceData *interfaceData);
    ...
    std::vector<bmi2_sens_axes_data> extractBuffer;
